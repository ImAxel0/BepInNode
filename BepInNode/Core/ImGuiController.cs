...
namespace BepInNode.Core
{
    public class ImGuiController : IDisposable
    {
        ...
        private unsafe static void LoadCustomFonts()
        {
            var io = ImGui.GetIO();
            Helpers.TryGetEmbeddedResourceBytes("Inter", out var fontData);
            GCHandle pinnedArray = GCHandle.Alloc(fontData, GCHandleType.Pinned);
            IntPtr pointer = pinnedArray.AddrOfPinnedObject();

            for (int i = 16; i < 100; i++)
            {
                FontController.Fonts.Add(ImGui.GetIO().Fonts.AddFontFromMemoryTTF(pointer, fontData.Length, i));
                if (i == 16 || i == 18 || i == 20 || i == 22)
                {
                    LoadIcons(i - 2);

                    ImFontConfig* config = io.Fonts.ConfigData[i - 16];
        ...
        private static unsafe void LoadIcons(float fontSize)
        {
